...
public class BulkImportProgramMember implements Function {
	private static final Logger logger = LoggerFactory.getLogger(BulkLeadImport.class);
	...
	public void execute(ExecutionParameters parameters) {
		...
		JsonString id = configuration.getJsonString("programId");
		if (id == null) {
			throw new IllegalStateException("program id  is required");
		}
		final String endpoint = "/bulk/v1/program/" + id.getString() + "/members/import.json?format=csv";
		...
		try {
			...
			String statusEndpoint = "/bulk/v1/program/members/import/" + batchId.getString() + "/status.json";
			...
			while (!sts1.equals("Completed")) {
				Thread.sleep(60000);
				String statusEndpoint1 = "/bulk/v1/program/members/import/" + batchId.getString() + "/status.json";
			...
			if (sts1.equals("Completed")) {
				if (fail > 0) {
					...
					String failendpoint = "/bulk/v1/program/members/import/" + batchId.getString() + "/failures.json";
				...
				if (warning > 0) {
					...
					String warningendpoint = "/bulk/v1/program/members/import/" + batchId.getString()
							+ "/warnings.json";
